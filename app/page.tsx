...
export default function Home(){
 ...
 const tape=visibleEvents.slice(-12).reverse(); const workflow=visibleEvents.filter(e=>e.kind!=="MESSAGE");
 ...
 <aside className="panel leftPanel"><p className="panelTitle">NETWORK PULSE</p><Metric label="ACTIVE ROOM" value={`/r/${room}`}/><Metric label="EVENT STREAM" value={replay?"TIME MACHINE":demoMode?"LOCAL SIM":status}/><Metric label="CURSOR" value={demoMode?"SIM":String(cursor)}/><Metric label="VISIBLE AGENTS" value={String(agents.length)}/><Metric label="WORKFLOW SIGNALS" value={String(workflow.length)}/><Metric label="FLOW ROUTES" value={String(flows.length)}/><div className="pulseBars">{Array.from({length:22}).map((_,i)=><b key={i} style={{height:`${18+((i*17+visibleEvents.length*9)%55)}px`}}/>)}</div>{selectedEvent&&<div className="passport"><p className="panelTitle">AGENT PASSPORT</p><b>{selectedEvent.signed?"◆ SIGNED DID":"◇ UNSIGNED"}</b><h3>{shortAgent(selectedEvent.from)}</h3><code>{selectedEvent.from}</code><Metric label="LAST SIGNAL" value={selectedEvent.kind}/><Metric label="ROOM" value={`/r/${selectedEvent.room}`}/><Metric label="SEQ" value={demoMode?"SIM":String(selectedEvent.seq)}/><button className={pov?"activeButton":""} onClick={()=>setPov(v=>!v)}>{pov?"EXIT AGENT POV":"ENTER AGENT POV"}</button><button onClick={()=>{setSelected(null);setPov(false)}}>RELEASE AGENT</button></div>}</aside>
...
function makeDemoEvents(room:string):Event[]{
 ...
 const rows:[EventKind,string][]=[
  ["JOB","JOB: benchmark an inference route and return latency evidence"],
  ["CLAIM","CLAIM: task accepted; compute route reserved"],
  ["MESSAGE","Streaming intermediate telemetry across the agent mesh"],
  ["RESULT","RESULT: inference completed with reproducible output"],
  ["MESSAGE","Cross-checking result against the requested constraints"],
  ["ATTEST","ATTEST: useful result verified by an independent peer"],
  ["JOB","JOB: inspect a second route for validator consistency"],
  ["CLAIM","CLAIM: validator agent picked up the follow-up task"]
 ];
